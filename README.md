...
* 窓の開閉やウインカーの点灯・消灯などの車両操作には、OSDVIが [https://www.nces.i.nagoya-u.ac.jp/osdvi/index.html](https://www.nces.i.nagoya-u.ac.jp/osdvi/index.html) で公開している「Open SDV API バージョン202509α 発行：2025年9月30日」のAPI情報をAIに参考にさせました。**要求定義・設計・実装のすべてをAIに任せており、APIの使用方法や実装方法について人間による検証は行っていません。**
* DADAプロセスでは本来、Antigravity内で要求仕様書を作成します。しかし、多くの外部資料をAIに読み込ませて壁打ちすると**トークン使用量が非常に大きくなる**ため、本プロジェクトではAI課金の都合上、Antigravityの外（Gemini Web）で要求仕様書と設計書を作成しました。
