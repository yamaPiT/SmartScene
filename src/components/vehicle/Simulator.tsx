...
/**
 * @file Simulator.tsx
 * @description 【シミュレータメインコンポーネント】
 * アプリケーションの全体レイアウトを統括し、物理ループ（Physics Loop）に基づく各種シミュレーションを実行する。
 * ダッシュボード、車両3Dモデル、制御パネルを統合し、時間軸に沿ったステータス更新（例：窓の開閉アニメーション等）を担う。
 * 
 * ■ ソフトウェア要求仕様書（SW105）とのトレーサビリティ:
 * - [REQ-V01] 3D車両モデルのリアルタイム表示
 * - [REQ-V03] アクチュエータ動作アニメーション（窓）
 * - 6.1 性能・効率性 (目標60fpsでの描画と更新)
 */
 ...
 * `useEffect` 内で100ms周期のメイン物理ループ（Physics Loop）を稼働させ、窓開閉のアニメーションや、
 * ウインカーの音の同期、点滅フラグのリレーション等を管理・処理する。
 * @returns {JSX.Element} シミュレータのUI全体
 */
