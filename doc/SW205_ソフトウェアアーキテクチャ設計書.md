...
* **Store ⇔ Logic間**:
  シナリオエンジンが自動制御イベント（例: 雨量10%エッジ検知）でアクションを発行した際、`Internal.ManualOverrideFlags` によって当該機能へのユーザー手動介入が検知された場合は、副作用なく更新がRejectされること。
...
* `Internal.UserMemoryState` / `PreRunStateCache` / `ManualOverrideFlags` : 既存踏襲。
